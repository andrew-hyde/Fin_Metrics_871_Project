...
data_alsi <- data_alsi %>% mutate(Return = log(Return)) %>% 
...
REITs to include: EMI, GRT, HYP, RDF, RES, SAC.
...
# REITs to include: EMI, GRT, HYP, RDF, RES, SAC
imputed_REIT_returns_spread <- imputed_REIT_returns_spread[, c(1,13,18,25,29,42,44,48)]
...
I now use the imputed data for the ALSI less REITs and make it tidy by placing the data into 3 columns (date, Tickers, Return). Next wrangle the data to calculate average daily returns of teh ALSI less REITs.
...
# gather the imputed, spread data and make it tidy

# REITs
imputed_REIT_returns_tidy <- imputed_REIT_returns_spread %>% 
...
# ALSI less REITs
imputed_ALSI_returns_tidy <- imputed_ALSI_returns_spread %>% 
...
# Bind the ALSI weighted return daily data (less property stocks)
# with the individual property stocks to be use for the DCC model
# Log and scale the data to align with statistical assumptions/principles, so that accurate inferences can be made

combined_data_ALSI_REIT <- bind_rows(imputed_ALSI_returns_tidy,
                                     imputed_REIT_returns_tidy) #%>%

#################
#   FIGURE OUT
##################
    mutate(log_return = log(Returns)) %>% # log the data
    mutate(log_scaled_weighted_return = # scale the logged weighted daily returns
            log_return - mean(log_return, na.rm = F)) %>%
    select(date, Ticker, log_scaled_weighted_return) %>%
    mutate(Return = log_scaled_weighted_return) %>% 
    arrange(date) %>%
    filter(date >= as.Date("2012-01-01") & date <= as.Date("2022-10-31")) %>% 
        spread(Tickers, Return)

# save the data as a backup
saveRDS(combined_data_ALSI_REIT, file = "data/my_data.rds")

################################################################################
# CODE THAT WAS BUILT EARLIER AND MAY BE USEFUL LATER
################################################################################
# data_combined_ALSI_REIT <- bind_rows(data_alsi_less_reit, property_returns) %>% 
#     mutate(include = ifelse(daily_weighted_returns > 0, "Include", "Exclude")) %>%
#     filter(include == "Include") %>%
#     mutate(log_return = log(daily_weighted_returns)) %>% # log the data 
#     mutate(log_scaled_weighted_return = # scale the logged weighted daily returns
#             log_return - mean(log_return, na.rm = F)) %>% 
#     select(date, Ticker, log_scaled_weighted_return) %>% 
#     arrange(date) %>% 
#     filter(date >= as.Date("2012-01-01") & date <= as.Date("2022-10-31")) %>% 
#     spread(Tickers, log_scaled_weighted_return)
